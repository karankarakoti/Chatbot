...
class Chatbot extends Component{
    ...
    async componentDidMount(){
        ...
        if(window.location.pathname==='/shop' && !this.state.welcomeSent)
        {
            ...
            this.df_event_query('WELCOME_HERE');
            this.setState({welcomeSent:true , showBot:true});
        }

        this.props.history.listen(()=>{
            
            if(this.props.history.location.pathname==='/shop' && !this.state.welcomeSent){
                this.df_event_query('WELCOME_HERE');
                this.setState({welcomeSent:true, showBot:true});
            }
        });
    ...
    render(){
        ...
        return(
            <div style={{minHeight:500,maxHeight:500, width:400, position:'absolute', bottom:0,right:0, border:'1px solid lightgrey'}}>
                ...
                <div id="chatbot" style={{maxHeight:388,minHeight:388,width:'100%', overflow:'auto'}}>
                ...
                <input style={{margin:0, paddingLeft:'1%', paddingRight:'1%', width:'98%'}} placeholder="Type a message" type="text" ref={(input)=>{this.talkInput=input;}} onKeyPress={this._handleInputKeyPress} id="user_says" />
                </div>
            </div>
        );
    }
